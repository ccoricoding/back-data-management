...
export const db = {
    ...
    async getCategories(year) {
        ...
        const categories = {};
        // If year is provided, filter keys starting with "[YEAR]_" or exact match if legacy
        data.forEach(row => {
            if (year) {
                const prefix = `${year}_`;
                ...
                // Return all for legacy/backup compatibility
    ...
    async getData(year) {
        ...
        if (year) {
            // Filter by year stored in overview JSONB
            query = query.eq('overview->>year', String(year));
        }
    ...
    async saveData(entry, year) {
        // Ensure year is saved in overview
        const overview = { ...entry.overview };
        if (year) overview.year = String(year);
    ...
    async updateData(entryId, updates, year) {
        const updateData = {
            updated_at: new Date().toISOString()
        };
        ...
        if (updates.overview !== undefined) {
            ...
            if (year) updateData.overview.year = String(year);
        }
